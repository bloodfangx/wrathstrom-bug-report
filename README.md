...
1.  **Search Existing Reports:** Prior to submitting a new ticket, consult the "Issues" tab to ensure the bug has not already been reported. Duplicate tickets will be closed.
2.  **Single Issue Per Ticket:** Do not combine multiple unrelated bugs into a single report. Each distinct issue requires its own submission for tracking purposes.
3.  **Specific Documentation:** Vague descriptions (e.g., "The quest is broken") provide insufficient data for debugging. Provide specific names, locations, and observed behaviors.
4.  **Database References:** Include relevant identification numbers (IDs) from Wowhead or our internal database to ensure accuracy.
